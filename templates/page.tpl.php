<?php
/**
 * Mosquito theme: main page template.
 *
 * Available variables:
 *
...
<div id="wrapper">
	...
	<header id="site-header" class="clear-block">
        ...
        <?php endif; // !empty($logo); ?>

        <?php if (!empty($site_name)): ?>
        <h1 id="site-name">
            <a href="<?php echo $front_page; ?>" title="<?php print t('Home'); ?>" rel="home"><?php echo $site_name; ?></a>
        </h1>
        <?php endif; // !empty($site_name); ?>

        <?php if (!empty($primary_links)): ?>
        <nav id="site-navigation">
        	<?php echo theme('links', $primary_links, array('class' => 'links primary-links')); ?>
        </nav>
        <?php endif; // !empty($primary_links); ?>
    ...
    <section id="site-content" class="clear-block">
    	...
    	<header id="content-header" class="clear-block">
            ...
            <?php endif; // !empty($page_title); ?>

            <?php if (!empty($tabs)): ?>
            <nav id="context-navigation">
            	<?php echo $tabs; ?>
            </nav>
            <?php endif; // !empty($tabs); ?>

        </header>
        <?php endif; // !empty($page_title) || !empty($tabs); ?>

        <section id="content" class="clear-block">

            <?php if (!empty($messages)): ?><section id="messages"><?php echo $messages; ?></section><?php endif; ?>
            <?php if (!empty($help)): ?><section id="help"><?php echo $help; ?></section><?php endif; ?>
        ...
        <section id="content-footer" class="clear-block">

            <?php if (!empty($feed_icons)): ?>
            <section id="feed-icons">
                <?php echo $feed_icons; ?>
            </section>
            <?php endif; // !empty($feed_icons); ?>

        </section>
        <?php endif; // !empty($content_footer); ?>
